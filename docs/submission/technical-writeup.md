...
**On-chain proof of the scoreline (`/verify`).** This uses TxLINE's actual differentiator: the data is off-chain, but a daily Merkle root of every score is anchored on Solana. Tap **◆ Verify on-chain** on any read-out (or send `/verify`) and Pitchwire fetches the Merkle proof for that scoreline from `/api/scores/stat-validation` and runs the program's `validate_stat_v2` as a **read-only `.view()`** — a simulation, no transaction, no signing cost, no custody. The Solana program verifies the proof against the on-chain daily root and returns a boolean: a correct scoreline verifies `true`, a tampered one verifies `false` (we confirmed both). The bot shows **✓ Verified on-chain** with a Solana Explorer link to the root. This is the settlement primitive used for on-chain trades, turned toward pure honest verification: we don't trade, we prove. It makes the trust story concrete — the number on screen is provably TxLINE's own, checked by the chain, not asserted by us.
